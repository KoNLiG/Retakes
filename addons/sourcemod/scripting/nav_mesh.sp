...
void CreateNatives()
{
    ...
    // int GetAdjacentCount(NavDirType dir)
    CreateNative("NavArea.GetAdjacentCount", Native_GetAdjacentCount);

    // NavArea GetAdjacentArea(NavDirType dir, int index)
    CreateNative("NavArea.GetAdjacentArea", Native_GetAdjacentArea);
...
any Native_GetAdjacentCount(Handle plugin, int numParams)
{
    NavArea nav_area = view_as<NavArea>(GetNativeCell(1));
    if (!nav_area)
    {
    ...
    // '4 *' as per array there are 4 bytes of data. (integer)
    ...
    // 'm_Size' offset is 0. [m_connect + 0]
    return LoadFromAddress(m_connect, NumberType_Int32);
}

any Native_GetAdjacentArea(Handle plugin, int numParams)
{
    NavArea nav_area = view_as<NavArea>(GetNativeCell(1));
    if (!nav_area)
    {
    ...
    return LoadFromAddress(m_connect + view_as<Address>(8) * view_as<Address>(index) + view_as<Address>(4), NumberType_Int32);
}
